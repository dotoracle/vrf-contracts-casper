...
>(
    ...
    let mut bytes_4 = t4.to_bytes().unwrap_or_revert();
    let mut bytes_5 = t5.to_bytes().unwrap_or_revert();
    let mut bytes_6 = t6.to_bytes().unwrap_or_revert();
    bytes_1.append(&mut bytes_2);
    bytes_1.append(&mut bytes_3);
    bytes_1.append(&mut bytes_4);
    bytes_1.append(&mut bytes_5);
...
pub fn mulmod(a: U256, b: U256, c: U256) -> U256 {
    let ret = u256_to_u512(a) * u256_to_u512(b);
    let ret = ret.div_mod(u256_to_u512(c)).1;
    U256::from_dec_str(&ret.to_string()).unwrap()
}

pub fn add_mod(a: U256, b: U256, c: U256) -> U256 {
    let ret = u256_to_u512(a) + u256_to_u512(b);
    let ret = ret.div_mod(u256_to_u512(c)).1;
    U256::from_dec_str(&ret.to_string()).unwrap()
}
...
pub fn to_vec_string(strs: &[&str]) -> Vec<String> {
...
pub fn set_last_updated_timestamp() {
    set_key("last_updated_timestamp", current_block_timestamp());
}
